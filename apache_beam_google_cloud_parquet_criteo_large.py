...
# LABEL_KEY = "clicked"
NUM_NUMERIC_FEATURES = 504
NUMERIC_FEATURE_KEYS = [f"col_{i+1}" for i in range(NUM_NUMERIC_FEATURES)]
CATEGORICAL_FEATURE_KEYS = [f"col_{i}" for i in range(NUM_NUMERIC_FEATURES + 1, 547)]
LABEL_KEY = "col_0"
...
class NegsToZeroLog(beam.DoFn):
  ...
  def process(self, element):
    ...
    for i, key in enumerate([LABEL_KEY] + NUMERIC_FEATURE_KEYS + CATEGORICAL_FEATURE_KEYS):
      val = element.get(key, "0")
