...
def getCriteoAdData(
    ...
    # Handle parquet files with numbered parts
    if not criteo_kaggle and datafile.endswith('.parquet'):
        # Extract base name without part number and extension
        base_name = lstr[-1].rsplit('_part_', 1)[0]
        d_file = base_name
        # Get the part number format (e.g., 0000)
        part_format = lstr[-1].split('_part_')[-1].split('.')[0]
        part_digits = len(part_format)
    else:
        ...
        if criteo_kaggle:
            ...
            if path.exists(datafile):
                ...
                with open(datafile, 'rb') as f:
                    ...
                    total_count = parquet_metadata.num_rows
                ...
                sys.exit("ERROR: Parquet file not found")
        else:
            # WARNING: The raw data consist of parquet files
            # Each file contains samples with 13 continuous and 26 categorical features
            for i in range(days):
                datafile_i = f"{d_path}{d_file}_part_{str(i).zfill(part_digits)}.parquet" if datafile.endswith('.parquet') else datafile + "_" + str(i)
                if path.exists(str(datafile_i)):
                    print("Reading data from path=%s" % (str(datafile_i)))
                    # Read parquet file metadata to get row count
                    total_per_file_count = pq.read_metadata(datafile_i).num_rows
                    total_per_file.append(total_per_file_count)
                    total_count += total_per_file_count
                else:
                    sys.exit(
                        "ERROR: Criteo Terabyte Dataset path is invalid; please check the parquet files exist"
                    )
    ...
    def process_one_file(
        ...
        for chunk in pd.read_parquet(datfile, chunksize=chunk_size):
            # Process each row in the chunk
            for k, row in chunk.iterrows():
                # sub-sample data by dropping zero targets, if needed
                target = np.int32(row['col_0'])
                if (
                    target == 0
                ...
                y[i] = target
                X_int[i] = np.array([row[f'col_{j}'] for j in range(1, 14)], dtype=np.int32)
                if max_ind_range > 0:
                    X_cat[i] = np.array(
                        [int(row[f'col_{j}'], 16) % max_ind_range for j in range(14, 40)],
                        dtype=np.int32,
                    )
                else:
                    X_cat[i] = np.array(
                        [int(row[f'col_{j}'], 16) for j in range(14, 40)],
                        dtype=np.int32,
                    )
        ...
        if dataset_multiprocessing:
            ...
            processes = [
                Process(
                    ...
                    args=(
                        f"{d_path}{d_file}_part_{str(i).zfill(part_digits)}.parquet" if not criteo_kaggle and datafile.endswith('.parquet')
                        else datafile + "_" + str(i) if not criteo_kaggle 
                        else npzfile + "_{0}".format(i),
                        npzfile,
                        i,
                        total_per_file[i],
                        dataset_multiprocessing,
...
def loadDataset(
    ...
    if dataset == "kaggle":
        ...
        print("Processing Terabyte dataset...")
    else:
